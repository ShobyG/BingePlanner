...
class CalenderSeriesEvent:
    def __init__(self, events, series_name, dates_list, watch_hours_list, episodes_list):
        self.__events = []
        self.__set_events(events)
        self.__watch_hours = watch_hours_list
        self.__episodes_list = episodes_list
    ...
    def __create_group_event(self, series_name, dates_list):
        for i in range(len(dates_list)):
            new_event = {
                'id': self.generate_next_id(),
                'groupId': series_name,
                'title': f"{series_name} watch_hours:{self.__watch_hours[i]} episode: {self.__episodes_list[i]}",
                'start': dates_list[i]
            }
