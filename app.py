...
from wtforms import StringField, FileField, PasswordField, SubmitField, IntegerField, DateField, TimeField, widgets, \
    SelectMultipleField
...
class SeriesEventForm(FlaskForm):
    ...
    start_date = DateField(label="When do you want to start watching", default=date.today(),
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            if request.method == "POST":
                ...
                watch_hours = [int(monday), int(tuesday), int(wednesday), int(thursday), int(friday), int(saturday),
                               int(sunday)]
                ...
                cse = CalenderSeriesEvent(events, f"{series_name}- Season:{season_no}", dates_lst, watch_hours_list,
                                          episode_list)
                events = cse.events
